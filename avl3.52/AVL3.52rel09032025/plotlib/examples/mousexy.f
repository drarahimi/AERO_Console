...
C
      ...
      ii = 0
      do j = 1, 100
        write(*,*) 'calling getcursorxy'
        call GETCURSORXY(xx,yy,cin)
        write(*,*) 'after getcursorxy ',xx,yy,cin
C
c        write(*,*) 'calling gw_curs'
c        call GW_CURS(xx,yy,ikey)
c        write(*,*) 'after gw_curs ',xx,yy,ikey
c        cin = char(ikey)
c        write(*,*) 'ikey cin ',ikey, cin
c        ikey2 = iand(127,ikey)
c        cin = char(ikey2)
...
C--- now read cursor and move point
 300  call pltall('Move point...',' ',' ')
      ...
      write(*,*) 'after GETCURSOXYC ',xx,yy,ibtn
