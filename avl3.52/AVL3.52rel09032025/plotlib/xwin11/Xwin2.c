...
void
GWXOPEN( int *xsizeroot, int *ysizeroot, int *depth )
...
//   int   *xstart, *ystart, *xsize, *ysize;
{
   ...
   /*  Fonts by name, not absolutely needed for plotlib operation
     char *fontname = "6x12"; 
     char *fontname = "-*-helvetica-medium-r-*-*-10-*-*-*-*-*-*-*";
     char *fontname = "-*-courier-medium-r-*-*-12-*-*-*-*-*-*-*";
   */
...
/* load the font -- the font is not required for normal operation
                    and these two statements cause BadName font not found
		    errors on some X windows system */
...
*/
void
GWXSTATUS( uint *xstart, uint *ystart, uint *xsize, uint *ysize)
//    unsigned int *xstart, *ystart, *xsize, *ysize;
{
...
*/
void
GWXSTRING( int *x, int *y, char *string, int *length )
//   int     *x, *y, *length;
//   char    *string;
