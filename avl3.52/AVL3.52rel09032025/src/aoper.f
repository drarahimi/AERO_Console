...
C
C------------------------------------------------------
      ELSE IF(COMAND.EQ.'FSB ') THEN
C------ print strip forces in body axes
        IF(LSOL) THEN
         CALL GETFILE(LU,COMARG)
C
         IF(LU.LE.-1) THEN
...
C
C------------------------------------------------------
      ELSE IF(COMAND.EQ.'OB  ') THEN
C------ off-body flow survey
        CALL OBOPER
C
C------------------------------------------------------
...
C------ Change reference data 
 89     XYZREF(1) = PARVAL(IPXCG,IRUN)
        XYZREF(2) = PARVAL(IPYCG,IRUN)
        XYZREF(3) = PARVAL(IPZCG,IRUN)
        CDREF     = PARVAL(IPCD0,IRUN)
C
        WRITE(*,2090) SREF,CREF,BREF,XYZREF,CDREF
 2090   FORMAT(/' ==========================='
     &         /'  S ref: ', G11.5,
     &         /'  C ref: ', G11.5,
     &         /'  B ref: ', G11.5,
...
C
        IF    (INDEX('Cc',ITEMC(1:1)).NE.0 .AND.
     &         INDEX('Dd',ITEMC(2:2)).NE.0) THEN
          IF(NINP.EQ.0) THEN
 97          WRITE(*,2100) 'Added CD0 profile drag'
           READ (*,*,ERR=97) CDREF
          ELSE
           CDREF = RINP(1)
          ENDIF
          PARVAL(IPCD0,IRUN) = CDREF
C
        ELSEIF(INDEX('Ss',ITEMC(1:1)).NE.0) THEN
          IF(NINP.EQ.0) THEN
...
C
        ELSEIF(INDEX('Xx',ITEMC(1:1)).NE.0) THEN
          IF(NINP.EQ.0) THEN
 94        WRITE(*,2100) 'reference Xref'
           READ (*,*,ERR=94) XREF
          ELSE
           XREF = RINP(1)
          ENDIF
          PARVAL(IPXCG,IRUN) = XREF
          LSOL = .FALSE.
          LSRD = .FALSE.
C
        ELSEIF(INDEX('Yy',ITEMC(1:1)).NE.0) THEN
          IF(NINP.EQ.0) THEN
 95        WRITE(*,2100) 'reference Yref'
           READ (*,*,ERR=95) YREF
          ELSE
           ZREF = RINP(1)
          ENDIF
          PARVAL(IPYCG,IRUN) = YREF
          LSOL = .FALSE.
          LSRD = .FALSE.
C
        ELSEIF(INDEX('Zz',ITEMC(1:1)).NE.0) THEN
          IF(NINP.EQ.0) THEN
 96        WRITE(*,2100) 'reference Zref'
           READ (*,*,ERR=96) ZREF
          ELSE
           ZREF = RINP(1)
          ENDIF
          PARVAL(IPZCG,IRUN) = ZREF
          LSOL = .FALSE.
          LSRD = .FALSE.
C
        ELSE
